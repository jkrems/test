...
`;
      ...
      const absoluteFilename =
        topUserFrame.url.startsWith('file://') ?
          fileURLToPath(topUserFrame.url) : topUserFrame.url;
      const context = `\
${void callStacks}

${scopes.filter(Boolean).join('\n')}

    at ${absoluteFilename}:${topUserFrame.location.lineNumber + 1}:${topUserFrame.location.columnNumber + 1}
${stmtDetails}
`;
  ...
  async attach() {
    ...
    session.on('Debugger.paused', ({ params }) => {
      ...
      function resume(error) {
        ...
        // TODO: support --inspect-on-failure
        //       one outstanding challenge is over-reporting of uncaught errors
